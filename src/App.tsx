...
export default function App() {
  ...
  return (
    <div id="app-root" className="bg-[#f0f3f8] min-h-screen py-6 px-4 font-sans text-slate-800 flex flex-col justify-between selection:bg-orange-200">
      ...
      {/* Supabase Integration Setup Guide & Status Dashboard */}
      <section id="supabase-status-guide" className="max-w-4xl mx-auto w-full bg-white rounded-3xl p-6 mb-8 shadow-sm border border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-50 pb-4 mb-4">
          <div className="flex items-center gap-3">
            <div className={`w-3.5 h-3.5 rounded-full ${supabaseStatus.configured ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'} shrink-0`} />
            <div>
              <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                Supabase Connection Status: 
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                  supabaseStatus.configured ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                }`}>
                  {supabaseStatus.configured ? 'Connected' : 'Local Fallback'}
                </span>
              </h2>
          ...
          <button
            ...
            onClick={() => {
              const next = !monthDropdownOpen; // reuse local toggle variable or just use a small inline inline handler
              // Since we don't have a setup-guide state variable, let's declare a state variable or simply use a local showGuide state
              // Let's do that cleanly.
            }}
            className="text-xs font-semibold px-4 py-2 bg-slate-50 hover:bg-slate-100 rounded-xl border border-slate-200 transition-all text-slate-600 cursor-pointer text-center"
          >
            {supabaseStatus.configured ? 'Show Config Specs' : 'How to Setup Secrets? 🔌'}
          </button>
        </div>

        {/* Setup instructions block */}
        <div id="supabase-setup-steps" className="space-y-4 animate-fade-in text-slate-700">
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-xs leading-relaxed">
            <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-1.5 text-xs">
              <Info className="w-4 h-4 text-slate-500" />
              Where do I configure my secrets?
            </h3>
            <ol className="list-decimal list-inside space-y-2 text-slate-600 pl-1">
              <li>
                Open the **Settings** or **Secrets** menu in the top-right / sidebar configuration of Google AI Studio.
              </li>
              <li>
                Create two new secret keys exactly matching these names:
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                  <div className="bg-white p-2.5 rounded-xl border border-slate-100 font-mono text-[10px] flex justify-between items-center shadow-inner">
                    <span className="text-rose-600 font-bold">SUPABASE_URL</span>
                    <span className="text-slate-400 select-all">e.g. https://yourproject.supabase.co</span>
                  </div>
                  <div className="bg-white p-2.5 rounded-xl border border-slate-100 font-mono text-[10px] flex justify-between items-center shadow-inner">
                    <span className="text-[#6366f1] font-bold">SUPABASE_ANON_KEY</span>
                    <span className="text-slate-400 select-all">anon/public client key</span>
                  </div>
                </div>
              </li>
              <li>
                Click **Save Secrets**. Our built-in Node.js API server will automatically pick up these variables, verify connection, and secure your proxy.
              </li>
            </ol>
          </div>

          <div className="bg-slate-900 text-slate-200 rounded-2xl p-4 font-mono text-[11px] border border-slate-800 shadow-md">
            <div className="flex justify-between items-center border-b border-slate-800 pb-2 mb-2 text-slate-400 text-[10px]">
              <span className="font-bold text-slate-300">📁 SQL EDITOR SCHEMA CREATION</span>
              <span className="bg-slate-800 px-2 py-0.5 rounded text-[9px] uppercase font-bold text-amber-400">Copy & Run in Supabase</span>
            </div>
            <pre className="overflow-x-auto whitespace-pre leading-relaxed text-emerald-400 scrollbar-thin">
